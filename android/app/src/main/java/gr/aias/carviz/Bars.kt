...
class Bars {
    ...
    /** Το πλάτος του κύματος σε εικονοστοιχεία — δες τη [layout]. */
    private var lWamp = 8f
    ...
    private fun layout(box: Rect, sh: Float) {
        ...
        // ΤΟ ΚΥΜΑ ΠΑΙΡΝΕΙ ΤΟΝ ΕΛΕΥΘΕΡΟ ΧΩΡΟ — ΜΟΝΟ ΟΤΑΝ ΤΟΝ ΕΧΕΙ.
        //
        // Το πλάτος του κύματος είναι `WAMP · διάμετρος`, σωστό όσο τη διάμετρο
        // την ορίζει το ύψος. Στη διαχωρισμένη οθόνη του MG όμως —579×618— την
        // ορίζει το πλάτος: πέντε τελείες πρέπει να χωρέσουν σε 579
        // εικονοστοιχεία, βγαίνουν 102, το κύμα κουνιέται ±31, και μένουν πάνω
        // από τετρακόσια εικονοστοιχεία μαύρο. Πέντε κουμπάκια χαμένα σε άδειο
        // πεδίο, ακριβώς όπως φάνηκε στη φωτογραφία από το αυτοκίνητο.
        //
        // Ο έλεγχος είναι το [squeezed] και όχι ο ελεύθερος χώρος. Πρώτη γραφή
        // κοίταζε μόνο τον χώρο, και θα άλλαζε και την πλήρη οθόνη: εκεί η
        // διάμετρος είναι 189 σε ύψος 540, άρα μένουν 175 ελεύθερα και το κύμα
        // θα πήγαινε από ±57 σε ±87. Πενήντα τοις εκατό αλλαγή σε εικόνα που
        // έχει ήδη εγκριθεί, από μια διόρθωση που δεν την αφορούσε.
        lWamp = if (squeezed) min(max(WAMP * dd, (vh - dd) * 0.25f), (vh - dd) * 0.5f)
                else WAMP * dd
        ...
        // ΤΟ ΚΕΝΤΡΟ ΕΙΝΑΙ ΤΗΣ ΟΘΟΝΗΣ, ΟΧΙ ΤΗΣ ΣΤΑΘΕΡΗΣ ΠΕΡΙΟΧΗΣ.
        //
        // Στο MG η σταθερή περιοχή αρχίζει 132 εικονοστοιχεία κάτω από την
        // κορυφή —εκεί κάθεται η μπάρα κατάστασης του Android Auto— και
        // τελειώνει μόλις 18 πάνω από τον πάτο. Κεντραρισμένη μέσα της, η
        // σύνθεση καθόταν στο 402 από τα 690, δηλαδή στο 58% του ύψους:
        // οπτικά χαμηλά, και φάνηκε με το πρώτο βλέμμα στο αυτοκίνητο. Το
        // κέντρο της επιφάνειας είναι 345.
        lCy = sh * 0.5f

        // ΚΑΙ ΤΟ ΤΑΒΑΝΙ ΜΕΤΡΙΕΤΑΙ ΑΠΟ ΕΚΕΙ, ΜΕ ΤΟ ΚΥΜΑ ΜΕΣΑ.
        //
        // Ο παλιός κανόνας ήταν `vh · 0,98 / dd` και αγνοούσε δύο πράγματα:
        // πού κάθεται το κέντρο, και ότι το κύμα μετακινεί την τελεία άλλα
        // ±57 πριν καν ψηλώσει. Με το κέντρο ανεβασμένο θα έβγαζε τις κορυφές
        // είκοσι τρία εικονοστοιχεία από την κορυφή της οθόνης.
        //
        // Το περιθώριο δένεται στη διάμετρο και όχι στο ύψος της οθόνης:
        // κλιμακώνεται με τη ζωγραφιά, όχι με το ταμπλό.
        val room = min(lCy, sh - lCy) - dd * 0.25f - lWamp
        lRmax = min(RMAX, max(1f, room * 2f / dd))
    }
